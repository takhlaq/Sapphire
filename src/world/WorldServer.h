...
namespace Sapphire::World
{
  ...
  public:
    ...
    void queueForPlayers( const std::set< uint64_t >& characterIds,
    ...
    void queueForLinkshell( uint64_t lsId, Sapphire::Network::Packets::FFXIVPacketBasePtr pPacket,
                            std::set< uint64_t > exceptionCharIdList = {} );

    void queueForFreeCompany( uint64_t fcId, Sapphire::Network::Packets::FFXIVPacketBasePtr pPacket,
                              std::set< uint64_t > exceptionCharIdList = {} );

    Sapphire::Common::Config::WorldConfig& getConfig();

  public:
    // Returns a snapshot of logged-in players in format "Name (CharacterID)" for GUI display
    std::vector< std::string > getLoggedInPlayersSnapshot();
  ...
  private:
    ...
    // Network hive running the IO service
    std::shared_ptr< Sapphire::Network::Hive > m_hive;
